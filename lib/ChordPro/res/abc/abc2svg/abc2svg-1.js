...
Abc.prototype.set_sym_glue=function(width){var g,x,some_grace,xmin=0,xx=0,xs=0,xse=0,ll=!tsnext||(tsnext.type==C.BLOCK&&!tsnext.play)||blocks.length,s=tsfirst,spf=(ll&&s.fmt.stretchlast)?s.fmt.stretchlast:(s.fmt.stretchstaff||s.fmt.maxshrink)
...
break}
if(s.space){if(s.space<s.shrink){xse+=s.shrink;xx+=s.shrink}else{xx+=s.space*(1-spf)
+s.shrink*spf}}else{xs+=s.shrink}}}
if(!xx){realwidth=0
return}
s=tsfirst
if(xmin>=width){x=0
...
x+=s.shrink;s.x=x}
spf_last=0}else if((ll&&xx/width>1-s.fmt.stretchlast)||(!ll&&(xx+xs>width||s.fmt.stretchstaff))){if(xx==xse)
xx+=5
...
abc2svg.version="6a97925c43";abc2svg.vdate="2023-12-02"
