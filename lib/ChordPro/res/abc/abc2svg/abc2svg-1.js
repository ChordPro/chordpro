...
if(de.cont)
new_de.push(de.start)}else if(dd.str!=undefined&&!tgls[dd.glyph]&&!glyphs[dd.glyph]){out_deco_str(x,y,de)}else if(de.lden){out_deco_long(x,y,de)}else{xygl(x,y,f)}
if(stv_g.g)
...
max_stem_err=0;s=s1
if(two_dir){n=1
for(;s!=s2;s=s.next){if(s.stem!=s1.stem&&(s.nflags==1||s.beam_br1||s.beam_br2)){n=0
break}}
if(n)
...
break}
if(s.space){if(s.space<s.shrink){xse+=s.shrink;xx+=s.shrink}else{xx+=s.space*spf+s.shrink*(1-spf)
xx0+=s.shrink}}else{xs+=s.shrink}}}
if(!xx){realwidth=0
return}
s=tsfirst
if(ll){if(((xx0+xx)/2+xs)/width<s.fmt.stretchlast)
stretch=1}else if(s.fmt.stretchstaff){stretch=1}
if(xmin>=width){x=0
for(;s;s=s.ts_next){if(s.seqst)
x+=s.shrink;s.x=x}
spf_last=0}else if((xx0+xx)/2+xs>width||stretch){while(--cnt>=0){if(xx==xse)
xx+=5
...
break}
spf_last=width/(xx+xs)}else{spf=spf_last
...
x=0
for(;s;s=s.ts_next){if(s.seqst)
x+=s.space?(s.space*spf<=s.shrink?s.shrink:s.space*spf):s.shrink
s.x=x}}
realwidth=x
...
posx+=indent}
draw_sym_near();line_height=set_staff();draw_systems(indent);draw_all_sym();delayed_update();if(output)
vskip(line_height)
if(indent)
posx-=indent}
blk_flush()
...
abc2svg.version="v1.22.11-27-b142d700e9";abc2svg.vdate="2023-12-21"
abc2svg.version="v1.22.12";abc2svg.vdate="2023-12-22"
