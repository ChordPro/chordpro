...
function do_combine(s) {
	var s2, s3, type, i, n, sl
		...
		// if two slurs in the same combined sequence, remove the lowest slur
		// (assume only one slur per voice)
		if (s.sls && s2.sls
		 && s.sls[0].se.time == s2.sls[0].se.time
		 && may_combine(s.sls[0].se))
			delete s2.sls

		// if some slurs start on the second symbol
		// move them to the combined symbol
		// and update the starting symbol of the ending slurs
		if (s2.sls) {
			if (s.sls)
