# generated by wxGlade 1.1.0b1 on Tue Sep 10 21:36:37 2024
#
...
sub new {
    ...
    $self->{p_edit} = ChordPro::Wx::EditorPanel->new($self->{nb_main}, wxID_ANY);
    $self->{nb_main}->AddPage($self->{p_edit}, _T("Song source"));
    
    $self->{p_sbexport} = ChordPro::Wx::SongbookExportPanel->new($self->{nb_main}, wxID_ANY);
    $self->{p_sbexport}->Show(0);
    $self->{nb_main}->AddPage($self->{p_sbexport}, _T("Songbook"));
    
    $self->{p_msg} = ChordPro::Wx::MessagesPanel->new($self->{nb_main}, wxID_ANY);
