# generated by wxGlade 1.1.0b1 on Mon Sep  9 22:42:09 2024
#
...
sub new {
    ...
    $self->SetSize($self->ConvertDialogSizeToPixels(Wx::Size->new(400, 301)));
    ...
    my $bm_init = Wx::StaticBitmap->new($self->{p_init}, wxID_ANY, Wx::Bitmap->new("/home/jv/src/ChordPro/lib/ChordPro/res/icons/chordpro-icon-narrow.png", wxBITMAP_TYPE_ANY));
    ...
    $self->{b_initexamople} = Wx::Button->new($self->{p_init}, wxID_ANY, "");
    ...
    $self->{l_initexample} = Wx::StaticText->new($self->{p_init}, wxID_ANY, _T("Open an example song"));
    ...
    $self->{nb_main}->Show(0);
    $self->{sz_main}->Add($self->{nb_main}, 1, wxEXPAND, 0);
    
    $self->{p_edit} = Wx::Panel->new($self->{nb_main}, wxID_ANY);
    $self->{p_edit}->SetFocus();
    $self->{nb_main}->AddPage($self->{p_edit}, _T("Song source"));
    
    $self->{sz_source} = Wx::BoxSizer->new(wxHORIZONTAL);
    
    $self->{t_source} = Wx::TextCtrl->new($self->{p_edit}, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE);
    $self->{sz_source}->Add($self->{t_source}, 1, wxEXPAND, 0);
    
    $self->{p_msg} = Wx::Panel->new($self->{nb_main}, wxID_ANY);
    $self->{nb_main}->AddPage($self->{p_msg}, _T("Messages"));
    
    $self->{sz_msg} = Wx::BoxSizer->new(wxVERTICAL);
    
    $self->{t_msg} = Wx::TextCtrl->new($self->{p_msg}, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxTE_READONLY);
    ...
    $self->{button_3} = Wx::Button->new($self->{p_msg}, wxID_CLEAR, "");
    $self->{sizer_1}->Add($self->{button_3}, 0, wxRIGHT, 5);
    
    $self->{button_1} = Wx::Button->new($self->{p_msg}, wxID_SAVE, "");
...
sub OnInitialExample {
    my ($self, $event) = @_;
    # wxGlade: ChordPro::Wx::Main_wxg::OnInitialExample <event_handler>
...
sub OnMsgClear {
    my ($self, $event) = @_;
    # wxGlade: ChordPro::Wx::Main_wxg::OnMsgClear <event_handler>
