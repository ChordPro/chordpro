# generated by wxGlade 1.1.0b1 on Fri Sep 13 14:58:59 2024
#
...
sub new {
    ...
    $self->SetSize($self->ConvertDialogSizeToPixels(Wx::Size->new(253, 231)));
    ...
    $self->{fp_customconfig} = Wx::FilePickerCtrl->new($self, wxID_ANY, "", _T("Choose config file"), _T("Config files (*.prp,*.json)|*.prp;*.json|All files|*.*"), wxDefaultPosition, wxDefaultSize, wxFLP_FILE_MUST_EXIST|wxFLP_OPEN|wxFLP_USE_TEXTCTRL);
    ...
    $self->{sizer_3}->Add($self->{sp_editfont}, 0, 0, 5);
    
    $self->{l_pdfviewer} = Wx::StaticText->new($self, wxID_ANY, _T("PDF previewer"));
    $self->{sizer_2}->Add($self->{l_pdfviewer}, Wx::GBPosition->new(11, 0), Wx::GBSpan->new(1, 1), wxALIGN_CENTER_VERTICAL, 5);
    ...
    $self->{sz_prefs_inner}->Add($self->{sz_prefs_buttons}, 0, wxALL|wxEXPAND, 5);
