...
`%{`_name_`}`, for example `%{title}`. The current page number can be
obtained with `%{page}`, and the song index in the songbook with `%{songindex}`. For a complete description on how to use metadata in heading strings, see [[here|ChordPro Configuration Format Strings]].
